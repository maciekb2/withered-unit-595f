...
- używa `chat()` (model gpt-5) z guardrails (temperature 0.3, top_p 0.9, max_tokens 800),
...
`generateDraft(outline, articlePrompt)` tworzy szkic:
 - korzysta z `chat()` (model gpt-5, temperature 0.6, top_p 0.9, max_tokens 1200) z wklejonym outline oraz regułami,
...
`editDraft(draft, outline)` wygładza tekst:
 - `chat()` (model gpt-5) z niską temperaturą 0.2 (top_p 0.9, max_tokens 1000),
...
`proofread(edited)` sprawdza gramatykę, styl i płynność całego tekstu:
 - `chat()` (model gpt-5) z temperaturą 0.2 (top_p 0.9, max_tokens 1000),
