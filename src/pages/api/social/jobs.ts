...
export const GET: APIRoute = async ({ request }) => {
  ...
  const result = await getPool().query(
    `SELECT j.id,j.slug,j.status,j.score,j.attempts,j.scheduled_for,j.last_error,j.created_at,j.updated_at,
...
export const POST: APIRoute = async ({ request }) => {
  ...
  const status = body.action === 'skip' ? 'skipped' : body.action === 'retry' || body.action === 'regenerate' ? 'eligible' : null;
